...
| ~~D3~~ | ~~Domain~~ | — | **Resolved 2026-08-24: `studioos.csdstudio.com`.** The product's public domain, hosted under CSD Studio's own company domain since they're the maker — not a private, single-tenant deployment (see D2). Feeds Phase 9b (DNS + Cloudflare Pages custom domain) and the OAuth app's registered redirect URI on `cloudshapeddreamsstudio.m.erpnext.com`, which must be updated from the current dev placeholder (`APP_ORIGIN = http://localhost:8787` in `backend/wrangler.toml`) to `https://studioos.csdstudio.com/auth/callback` before first deploy |
